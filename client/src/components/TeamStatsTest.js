...
class TeamStatsTest extends React.Component {
            ...
            .then((value) => {
                if (value.data.message === 'success') {
                    this.setState({
                        ...
                        MPS_95_VEL_DATA: [],
                        PLAYERS_POSITIONS: [],
                        BRAIN_POSITIONS: { 'principal-max-strain': [], 'principal-min-strain': [] }
                    });

                    getFilterdTeamSpheresTest({ brand: '', organization: 'Army Research Laboratory', team: ["2020 POMPOC Study"], filter: 'greater', gs: 0, type: 'principal-max-strain' })
                        .then(response => {
                            this.setState({
                                brainRegions: ''
                            })
    ...
    handleChange = (e) => {
        ...
        if(e.target.name === 'gs'){
            this.handleRunReport(e.target.value);
        }
    };
    handleRunReport = (gs) => {
      
        this.setState({ isfetching: true })
        getFilterdTeamSpheresTest({ brand: '', organization: 'Army Research Laboratory', team: ["2020 POMPOC Study"], filter: this.state.filter, gs: gs, type: this.state['principal-max-strain'] })
        .then(response => {
