...
class ListUserTableRow extends React.Component {
  ...
  toggleUserAccountStatus=(event)=>{
    ...
    if(this.isEnabled()){
      ...
      disableUser(JSON.stringify({user_name : this.state.user_cognito_id})).then((response)=>{
      ...
      // api to enable user
      enableUser(JSON.stringify({user_name : this.state.user_cognito_id})).then((response)=>{
        if(response.data.message == "success"){
          this.setState({
            Enabled : true
          })
...
}}>{this.getAccountStatus()}</button></td>
      </tr>
    </React.Fragment>;
  }
}
